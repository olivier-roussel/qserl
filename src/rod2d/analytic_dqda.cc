...
bool computeMotionConstantsDqDa(const Eigen::Vector3d& i_a, MotionConstantsDqDa& o_mc)
{
  ...
  o_mc.dlambda4_da = Eigen::Vector3d(-i_a[0] * o_mc.lambda[1], sqrd_a3, 
    2*i_a[2]);
  ...
  const double inv_delta = 1. / o_mc.delta;
  ...
  if (o_mc.lambda[3] >= 0.)
  {
    ...
    if (!(o_mc.lambda[3] == 0. && o_mc.lambda[1] < 0.))
    {
      ...
      o_mc.m = 0.5 + (o_mc.lambda[1] * 0.5 * inv_sqrt_delta);
      o_mc.k = sqrt(o_mc.m);
      o_mc.dm_da = 0.5 * inv_sqrt_delta * (o_mc.dlambda2_da - o_mc.lambda[1] * 0.5 * inv_delta * 
        o_mc.ddelta_da);
      o_mc.n = 1.;
      o_mc.dn_da = Eigen::Vector3d::Zero();
      o_mc.r = sqrt_4_delta * inv_sqrt_2;
      o_mc.dr_da = 0.25 * inv_sqrt_2 * inv_sqrt_4_delta * inv_sqrt_delta * o_mc.ddelta_da;

      // pre-compute some usefull constants
      const double m1 = 1. - o_mc.m;
      const double inv_r = 1. / o_mc.r;
      const double inv_alpha3 = 1. / o_mc.alpha[2];

      // compute the phase tau
      const double eta_sqrd = util::clamp(1. - sqrd_a3 * inv_alpha3, 0., 1.);
      o_mc.eta = sqrt(eta_sqrd);
      const double inv_eta = 1. / o_mc.eta;
      ...
      o_mc.dgamma_0_da = o_mc.dr_da * o_mc.tau + o_mc.dtau_da * o_mc.r;
      o_mc.sn_gamma_0 = boost::math::jacobi_elliptic(o_mc.k, o_mc.gamma_0, &o_mc.cn_gamma_0, &o_mc.dn_gamma_0); 
      const double cd_gamma_0 = o_mc.cn_gamma_0 / o_mc.dn_gamma_0;
      const double sc_gamma_0 = o_mc.sn_gamma_0 / o_mc.cn_gamma_0;
      ...
      // TODO elliptics integrals of the 1st and second kind code can be factorized for the same parameters
      o_mc.E_am_gamma_0 = boost::math::ellint_2(o_mc.k, am_gamma_0);

      // ddn_gamma_0_da
      const double ddn_gamma_0_dgamma_0 = -o_mc.m * o_mc.sn_gamma_0 * o_mc.cn_gamma_0;
      const double ddn_gamma_0_dm = (1./(2.*m1)) * (o_mc.sn_gamma_0 * o_mc.cn_gamma_0 * ((o_mc.m-1)*o_mc.gamma_0 + 
        o_mc.E_am_gamma_0 - o_mc.dn_gamma_0*sc_gamma_0));
      o_mc.ddn_gamma_0_da = ddn_gamma_0_dgamma_0 * o_mc.dgamma_0_da + ddn_gamma_0_dm * o_mc.dm_da;

      // dsn_gamma_0_da
      const double dsn_gamma_0_dgamma_0 = o_mc.cn_gamma_0 * o_mc.dn_gamma_0;
      const double dsn_gamma_0_dm = (1./(2.*o_mc.m*m1)) * (o_mc.dn_gamma_0*o_mc.cn_gamma_0*(m1*o_mc.gamma_0 - 
        o_mc.E_am_gamma_0 + o_mc.m*cd_gamma_0*o_mc.sn_gamma_0));
      ...
      const double dcn_gamma_0_dgamma_0 = -o_mc.sn_gamma_0 * o_mc.dn_gamma_0;
      const double dcn_gamma_0_dm = (1./(2.*o_mc.m*m1)) * (o_mc.sn_gamma_0*o_mc.dn_gamma_0 * ((o_mc.m-1)*o_mc.gamma_0 + 
        o_mc.E_am_gamma_0 - o_mc.m*o_mc.sn_gamma_0*cd_gamma_0));
      ...
      const double dE_am_gamma_0_dam_gamma_0 = o_mc.dn_gamma_0;
      const double dam_gamma_0_dgamma_0 = o_mc.dn_gamma_0;
      const double dam_gamma_0_dm = (1./(2.*o_mc.m*(o_mc.m-1)))*(((o_mc.m-1) * o_mc.gamma_0 + o_mc.E_am_gamma_0)*
        o_mc.dn_gamma_0 - o_mc.m*o_mc.cn_gamma_0*o_mc.sn_gamma_0);
      const Eigen::Vector3d dam_gamma_0_da = dam_gamma_0_dgamma_0 * o_mc.dgamma_0_da + dam_gamma_0_dm * o_mc.dm_da;
      const double dE_am_gamma_0_dm = (o_mc.E_am_gamma_0 - F_am_gamma_0) / (2.*o_mc.m);
      ...
      o_mc.dbeta1_0_da = 4. * o_mc.dn_gamma_0 * o_mc.ddn_gamma_0_da;
      o_mc.dbeta2_0_da = o_mc.dsn_gamma_0_da * o_mc.dn_gamma_0 + o_mc.ddn_gamma_0_da * o_mc.sn_gamma_0;
    ...
    if (abs(i_a[1]) > kEpsilonNullForce || abs(i_a[2]) > kEpsilonNullForce)
    {
      ...
      // compute m, n and r (elliptic parameters) and its derivatives
      o_mc.m = (o_mc.alpha[2] - o_mc.alpha[1]) * inv_alpha3;
      o_mc.k = sqrt(o_mc.m);
      o_mc.dm_da = inv_sqrd_alpha3 * (o_mc.alpha[2] * inv_sqrt_delta - 1.) * o_mc.ddelta_da
      ...
      o_mc.deta_da = 0.5 * inv_eta * inv_n * ( sqrd_a3 * inv_sqrd_alpha3 * 
            (o_mc.dalpha_da.col(2) - Eigen::Vector3d(2.*o_mc.alpha[2] / i_a[0], 0., 0.)) - 
      ...
      // ddn_gamma_0_da
      const double ddn_gamma_0_dgamma_0 = -o_mc.m * o_mc.sn_gamma_0 * o_mc.cn_gamma_0;
      const double ddn_gamma_0_dm = (1./(2.*m1)) * (o_mc.sn_gamma_0 * o_mc.cn_gamma_0 * ((o_mc.m-1) * 
        o_mc.gamma_0 + o_mc.E_am_gamma_0 - o_mc.dn_gamma_0 * sc_gamma_0));
      o_mc.ddn_gamma_0_da = ddn_gamma_0_dgamma_0 * o_mc.dgamma_0_da + ddn_gamma_0_dm * o_mc.dm_da;

      // dcn_gamma_0_da
      const double dcn_gamma_0_dgamma_0 = -o_mc.sn_gamma_0 * o_mc.dn_gamma_0;
      const double dcn_gamma_0_dm = (1./(2.*o_mc.m*m1)) * (o_mc.sn_gamma_0*o_mc.dn_gamma_0 * 
        ((o_mc.m-1) * o_mc.gamma_0 + o_mc.E_am_gamma_0 - o_mc.m * o_mc.sn_gamma_0 * cd_gamma_0));
      o_mc.dcn_gamma_0_da = dcn_gamma_0_dgamma_0 * o_mc.dgamma_0_da + dcn_gamma_0_dm * o_mc.dm_da;
      ...
      o_mc.beta2_0 = o_mc.sn_gamma_0 * o_mc.cn_gamma_0;
      o_mc.dbeta1_0_da = -4. * o_mc.sn_gamma_0 * o_mc.dsn_gamma_0_da;
      o_mc.dbeta2_0_da = o_mc.dsn_gamma_0_da * o_mc.cn_gamma_0 + o_mc.dcn_gamma_0_da * o_mc.sn_gamma_0;
    }else{
...
bool computeDqDaAtPositionT(double i_t, const MotionConstantsDqDa& i_mc, Eigen::Matrix3d& o_dqda)
{
  ...
  if (i_mc.lambda[3] >= 0.)
  {
    ...
    if (!(i_mc.lambda[3] == 0. && i_mc.lambda[1] < 0.))
    {
      ...
      // derivatives
      // ddn_gamma_t_da
      const double ddn_gamma_t_dgamma_t = -i_mc.m * sn_gamma_t * cn_gamma_t;
      const double ddn_gamma_t_dm = (1./(2.*m1)) * (sn_gamma_t * cn_gamma_t * ((i_mc.m-1)*gamma_t +
        E_am_gamma_t - dn_gamma_t*sc_gamma_t));
      ...
      const double dcn_gamma_t_dm = (1./(2.*i_mc.m*m1)) * (sn_gamma_t * dn_gamma_t * ((i_mc.m-1)*gamma_t + 
        E_am_gamma_t - i_mc.m * sn_gamma_t * cd_gamma_t));
      ...
      const double dam_gamma_t_dm = (1./(2. * i_mc.m * (i_mc.m-1))) * (((i_mc.m-1) * gamma_t + E_am_gamma_t) 
        * dn_gamma_t - i_mc.m * cn_gamma_t * sn_gamma_t);
      const Eigen::Vector3d dam_gamma_t_da = dam_gamma_t_dgamma_t * dgamma_t_da + dam_gamma_t_dm * i_mc.dm_da;
      const double dE_am_gamma_t_dm = (E_am_gamma_t - F_am_gamma_t) / (2. * i_mc.m);
      ...
      const Eigen::Vector3d dint_beta1_da = inv_r * (-(1./(2.*i_mc.delta)) * i_mc.ddelta_da * 
        (E_am_gamma_t - i_mc.E_am_gamma_0) + 2. * (dE_am_gamma_t_da - i_mc.dE_am_gamma_0_da));
      const Eigen::Vector3d dint_beta2_da = inv_r * (inv_r * i_mc.dr_da * (cn_gamma_t - i_mc.cn_gamma_0) - 
            dcn_gamma_t_da + i_mc.dcn_gamma_0_da);

      // dq1 / da (q1 is angle theta)
      o_dqda.col(0) = 2. * i_mc.epsilon_k * ( 1. / (i_mc.k * i_mc.sn_gamma_0) ) * i_mc.ddn_gamma_0_da - 
        ( 1. / (i_mc.k * sn_gamma_t) ) * ddn_gamma_t_da;

      // dq2 / da (q2 is x position)
      o_dqda.col(1) = i_mc.dbeta1_0_da * int_beta1 + dint_beta1_da * i_mc.beta1_0 + 
        4 * (i_mc.dm_da * i_mc.beta2_0 + i_mc.dbeta2_0_da * i_mc.m * int_beta2 + dint_beta2_da * 
        i_mc.m * i_mc.beta2_0);

      // dq3 / da (q3 is y position)
      o_dqda.col(2) = i_mc.epsilon_k * ((1. / i_mc.k) * i_mc.dm_da * (i_mc.beta1_0 * int_beta2 - 
        i_mc.beta2_0*int_beta1) + 2 * i_mc.k * (i_mc.dbeta1_0_da * int_beta2 + dint_beta2_da * 
        i_mc.beta1_0 - i_mc.dbeta2_0_da * int_beta1 - dint_beta1_da * i_mc.beta2_0));
    ...
    if (i_mc.k != 0.)
    {
      ...
      const double int_beta1_p = i_t * (i_mc.m - 2.) + 2. * inv_r * (E_am_gamma_t - i_mc.E_am_gamma_0);
      ...
      const double dsn_gamma_t_dm = (1./(2. * i_mc.m * m1)) * (dn_gamma_t * cn_gamma_t * (m1 *gamma_t - 
        E_am_gamma_t + i_mc.m * cd_gamma_t * sn_gamma_t));
      const Eigen::Vector3d dsn_gamma_t_da = dsn_gamma_t_dgamma_t * dgamma_t_da + dsn_gamma_t_dm * i_mc.dm_da;
        
      // dE_am_gamma_t_da
      const double dE_am_gamma_t_dam_gamma_t = dn_gamma_t;
      const double dam_gamma_t_dgamma_t = dn_gamma_t;
      const double dam_gamma_t_dm = (1./(2. * i_mc.m * (i_mc.m-1)))*(((i_mc.m-1) * gamma_t + E_am_gamma_t) * dn_gamma_t - 
        i_mc.m * cn_gamma_t * sn_gamma_t);
      const Eigen::Vector3d dam_gamma_t_da = dam_gamma_t_dgamma_t * dgamma_t_da + dam_gamma_t_dm * i_mc.dm_da;
      const double dE_am_gamma_t_dm = (E_am_gamma_t - F_am_gamma_t) / (2. * i_mc.m);
      const Eigen::Vector3d dE_am_gamma_t_da = dE_am_gamma_t_dam_gamma_t * dam_gamma_t_da + dE_am_gamma_t_dm * i_mc.dm_da;
      ...
      const Eigen::Vector3d dint_beta1_p_da = i_t * i_mc.dm_da + 2. * inv_r * (-inv_r * i_mc.dr_da * 
        (E_am_gamma_t - i_mc.E_am_gamma_0) + dE_am_gamma_t_da - i_mc.dE_am_gamma_0_da);
      const Eigen::Vector3d dint_beta1_da = inv_m * (-inv_m * i_mc.dm_da * int_beta1_p + dint_beta1_p_da);

      // dint_beta2_da
      const Eigen::Vector3d dint_beta2_da = inv_m * inv_r * ((dn_gamma_t - i_mc.dn_gamma_0) *
        (inv_m * i_mc.dm_da + inv_r * i_mc.dr_da) - (ddn_gamma_t_da - i_mc.ddn_gamma_0_da));

      // dq1 / da (q1 is angle theta)
      o_dqda.col(0) = 2. * i_mc.epsilon_k * ( (1. / cn_gamma_t) * dsn_gamma_t_da - 
        (1. / i_mc.cn_gamma_0) * i_mc.dsn_gamma_0_da);

      // dq2 / da (q2 is x position)
      o_dqda.col(1) = i_mc.dbeta1_0_da * int_beta1 + i_mc.beta1_0 * dint_beta1_da + 
            4. * (i_mc.dbeta2_0_da * int_beta2 + i_mc.beta2_0 * dint_beta2_da);

      // dq3 / da (q3 is y position)
      o_dqda.col(2) = 2. * i_mc.epsilon_k *(i_mc.dbeta1_0_da * int_beta2 + i_mc.beta1_0 * 
            dint_beta2_da - i_mc.dbeta2_0_da * int_beta1 - i_mc.beta2_0 * dint_beta1_da);
    }else{
      // case II.2 : a4 == 0  and a5 == 0 (i.e. m == 0)
