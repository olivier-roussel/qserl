...
bool computeMotionConstantsDqDa(const Eigen::Vector3d& i_a, MotionConstantsDqDa& o_mc)
{
  ...
  if (o_mc.lambda[3] >= 0.)
  {
    ...
    if (!(o_mc.lambda[3] == 0. && o_mc.lambda[1] < 0.))
    {
      ...
      const double F_am_gamma_0 = o_mc.gamma_0;
...
bool computeDqDaAtPositionT(double i_t, const MotionConstantsDqDa& i_mc, Eigen::Matrix3d& o_dqda)
{
  ...
  if (i_mc.lambda[3] >= 0.)
  {
    ...
    if (!(i_mc.lambda[3] == 0. && i_mc.lambda[1] < 0.))
    {
      ...
      const double sn_gamma_t = boost::math::jacobi_elliptic(i_mc.k, gamma_t, &cn_gamma_t, &dn_gamma_t, &am_gamma_t); 
      ...
      o_dqda.row(0) = (2. * i_mc.epsilon_k * (( 1. / (i_mc.k * i_mc.sn_gamma_0) ) * i_mc.ddn_gamma_0_da - 
        ( 1. / (i_mc.k * sn_gamma_t) ) * ddn_gamma_t_da)).transpose();

      // dq2 / da (q2 is x position)
      o_dqda.row(1) = (i_mc.dbeta1_0_da * int_beta1 + dint_beta1_da * i_mc.beta1_0 + 
        4 * (int_beta2 * (i_mc.dm_da * i_mc.beta2_0 + i_mc.dbeta2_0_da * i_mc.m ) + dint_beta2_da * 
        i_mc.m * i_mc.beta2_0)).transpose();

      // dq3 / da (q3 is y position)
      o_dqda.row(2) = (i_mc.epsilon_k * ((1. / i_mc.k) * i_mc.dm_da * (i_mc.beta1_0 * int_beta2 - 
    ...
    if (i_mc.k != 0.)
    {
      ...
      // dq1 / da (q1 is angle theta)
      o_dqda.row(0) = (2 * i_mc.epsilon_k * ( (1. / cn_gamma_t) * dsn_gamma_t_da - 
        (1. / i_mc.cn_gamma_0) * i_mc.dsn_gamma_0_da)).transpose();

      // dq2 / da (q2 is x position)
      o_dqda.row(1) = (i_mc.dbeta1_0_da * int_beta1 + i_mc.beta1_0 * dint_beta1_da + 
            4. * (i_mc.dbeta2_0_da * int_beta2 + i_mc.beta2_0 * dint_beta2_da)).transpose();
