...
/*													Constructor																	*/
/************************************************************************/
WorkspaceIntegratedState::WorkspaceIntegratedState(const Displacement2D& i_basePosition,
...
{
  assert (m_rodParameters.delta_t > 0. and "step integration time must be stricly positive");
  m_numNodes = m_rodParameters.numberOfNodes();
  assert (m_numNodes > 1 and "rod number of nodes must be greater or equal to 2");
}
...
WorkspaceIntegratedState::create(const Wrench2D& i_baseWrench,
...
/************************************************************************/
bool
WorkspaceIntegratedState::init(const Wrench2D& i_wrench)
{
  bool success = true;
...
/************************************************************************/
WorkspaceIntegratedState::IntegrationResultT
WorkspaceIntegratedState::integrateWhileValid(const Wrench2D& i_maxWrench,
                                              double& o_tinv)
{
  ...
  while(isStable and not isOutOfWrenchBounds and iter < maxIter)
  {
    // integrate co-state
    css_stepper.do_step(costate_system, mu_t, t, dt);
  ...
  if(not m_integrationOptions.keepMuValues)
  {
    delete mu_buffer;
  }
